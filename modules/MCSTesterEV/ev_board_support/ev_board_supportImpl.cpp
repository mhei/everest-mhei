...
void ev_board_supportImpl::init() {
    // pre-init to current value
    this->bsp_measurement.cp_pwm_duty_cycle = this->mod->controller->get_cached_duty_cycle();

    this->mod->controller->on_pp_change.connect([&](const types::board_support_common::Ampacity& new_ampacity) {
        if (new_ampacity == Ampacity::None and this->pp_ampacity.has_value() and
            this->pp_ampacity.value().ampacity != Ampacity::None) {
            EVLOG_info << "PP noticed plug removal from socket";
            this->bsp_measurement.cp_pwm_duty_cycle = 100.f;
        } else {
    ...
    this->mod->controller->on_cp_change.connect([&](types::board_support_common::Event& event) {
        if (this->pp_ampacity.has_value() and this->pp_ampacity.value().ampacity == Ampacity::None and
            event == Event::A) {
            // reset CP state to Disconnected when we detected plug removal before
    ...
    this->mod->controller->on_duty_cycle_change.connect([&](const float& new_duty_cycle) {
        if (this->bsp_measurement.cp_pwm_duty_cycle != new_duty_cycle) {
