...
public:
    ...
    ev_board_supportImpl(Everest::ModuleAdapter* ev, const Everest::PtrContainer<EVAchargeSEEV>& mod, Conf& config) :
...
private:
    const Everest::PtrContainer<EVAchargeSEEV>& mod;
    const Conf& config;
    ...
    /// @brief Cached requested CP state
    types::ev_board_support::EvCpState requested_cp_state{types::ev_board_support::EvCpState::A};
    ...
    /// @brief Last detected ampacity
    std::optional<types::board_support_common::ProximityPilot> pp_ampacity;

    /// @brief Last published measurement values
    types::board_support_common::BspMeasurement bsp_measurement;
